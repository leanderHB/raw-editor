...
try {
  ...
  console.log('--- default mode (stageABC) via real file input ---');
  console.time('decode default');
  await page.setInputFiles('#fileInput', arwPath);
  await page.waitForFunction(() => document.getElementById('status').textContent.includes('stageABC'), { timeout: 30000 });
  console.timeEnd('decode default');
  console.log('status:', await page.evaluate(() => document.getElementById('status').textContent));
  await writeFile('./test_stageABC.jpg', Buffer.from((await page.evaluate(() => window.__testCapture())).split(',')[1], 'base64'));
  ...
  for (const mode of ['original', 'stageA', 'stageAB', 'stageABC']) {
